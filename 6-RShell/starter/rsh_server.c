	...
	//ec
 	if (is_threaded) {
        while (1) {
            ...
            *cli = accept(svr_socket, (struct sockaddr *)&cli_addr, &cli_len);
            if (*cli < 0) {
                perror("accept");
                free(cli);
                continue;
            ...
            pthread_t thread_id;
            if (pthread_create(&thread_id, NULL, handleClient, cli) != 0) {
...
int boot_server(char *ifaces, int port){
    ...
    int enable = 1;
    setsockopt(svr, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));
    ...
    ret = bind(svr, (struct sockaddr *)&addr, sizeof(addr));
    if (ret < 0) {
        perror("bind");
        close(svr);
        return ERR_RDSH_COMMUNICATION;
...
int process_cli_requests(int svr_socket){
    ...
    while (1) {
        struct sockaddr_in cli_addr;
        socklen_t cli_len = sizeof(cli_addr);
        cliSocket = accept(svr_socket, (struct sockaddr *)&cli_addr, &cli_len);
        if (cliSocket < 0) {
            perror("accept");
            return ERR_RDSH_COMMUNICATION;
        }
        rc = exec_client_requests(cliSocket);
        if (rc == OK_EXIT) {
            break;
        }
        close(cliSocket);
...
int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        if (build_cmd_buff(io_buff, &cmd_list) != OK) {
            send_message_string(cli_socket, "Error: Failed to parse command\n");
            continue;
        }
        int rc = rsh_execute_pipeline(cli_socket, &cmd_list);
        char result_buff[RDSH_COMM_BUFF_SZ];
        snprintf(result_buff, sizeof(result_buff), "Command executed with exit code: %d\n", rc);
        send_message_string(cli_socket, result_buff);
...
int send_message_eof(int cli_socket){
    ssize_t len;
    len = send(cli_socket, &RDSH_EOF_CHAR, 1, 0);
    if (len != 1) {
        perror("send error");
        return ERR_RDSH_COMMUNICATION;
    }
...
int send_message_string(int cli_socket, char *buff){
    ssize_t len;
    len = send(cli_socket, buff, strlen(buff), 0);
    if (len < 0) {
        perror("send error");
        return ERR_RDSH_COMMUNICATION;
    }
